...
class Module(ABC):
    ...
    """
    def __init__(self, info, options, auto_check=True):
...
class AgentModule(Module):
    ...
    def __init__(self, info, options, auto_check=True):
        options["agent"] =  Option.create(name="agent", description="Set Agent to send the task", required=True)
        super(AgentModule, self).__init__(info, options, auto_check)
    ...
    def update_complete_set(self):
        ...
        for a in agents:
            if a[-2] == status.ALIVE:
                ids.append(f"{a[0]}--{a[1]}")
        s_options.add_set_option_values("agent", ids)

    
    @exception("Error when assigning value (use tab to help)")
    def set_value(self, data):
        new_data = []
        key = data[0]
        new_data.append(key)
        try:
            if key == "agent":
                value = ' '.join([str(x) for x in data[1:]])
                new_data.append(value.split("--")[0])
            else:
                new_data = data
        except:
            new_data = data
        super(AgentModule, self).set_value(new_data)
