...
class RedTeamModule(AgentModule):
    ...
    def __init__(self):
        ...
        options =  {
            ...
            "args": Option.create(name="args", description="Arguments to pass to the Plugin (Example: URI, string, ...", required=True)
            }
        self.localport = 0
    ...
    def update_complete_set(self):
        ...
        plugins = []
        for (p, d, files) in walk("plugins/args"):
            for f in files:
    ...
    def run(self):
        try:
            ...
            if result == 0:
                ...
                uri_args = f"http://{self.args['lhost']}:{self.args['lport']}/args/ {self.args['plugin']}"
                rt_db.add_task(self.args["agent"], "LoadPlugin", uri_args, self.args["args"], "function")
            else:
